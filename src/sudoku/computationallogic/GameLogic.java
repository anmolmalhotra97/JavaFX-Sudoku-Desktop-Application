...
public class GameLogic {
    ...
    private static boolean rowsOfSquareisInvalid(Rows rowValue, int[][] grid) {
        switch (rowValue) {
            case TOP: {
    ...
    private static boolean collectionHasRepeats(List<Integer> collectionOfElementsInASquare) {
        for (int index = 1; index <= GRID_BOUNDARY; index++) {
            if (Collections.frequency(collectionOfElementsInASquare, index) > 1) {
                return true;
            }
        }
        return false;
