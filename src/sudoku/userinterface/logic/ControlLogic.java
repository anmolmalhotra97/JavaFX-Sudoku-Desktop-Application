...
public class ControlLogic implements IUserInterfaceContract.EventListener {
    ...
    public void onSudokuInput(int x, int y, int input) {
        try {
            ...
            //using storage as the only point to save and update new data.
            gameData = new SudokuGame(GameLogic.checkForCompletion(newGridState), newGridState);
            storage.updateGameData(gameData);

            //updating only single tile instead of complete board.
            view.updateSquare(x, y, input);

            if (GameState.COMPLETE.equals(gameData.getGameState())) {
